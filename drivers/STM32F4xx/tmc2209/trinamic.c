        ...
        if(idx < N_AXIS) switch((base_idx - idx) / AXIS_SETTINGS_INCREMENT) {
            ...
            case AxisSetting_MicroSteps:
                if(TMC2209_MicrostepsIsValid((uint16_t)value)) {
                    ...
                    TMC2209_ReadRegister(&stepper[idx], (TMC2209_datagram_t *)&stepper[idx].ifcnt);
                }
                else
    ...
    switch(gc_block->user_mcode) {
        ...
        case Trinamic_WriteRegister:
            if(bit_istrue(*value_words, bit(Word_R)) && bit_istrue(*value_words, bit(Word_Q)) /* && getAdr() != 0xFF)*/) {
                reg_ptr = TMC2209_GetRegPtr(&stepper[report.sg_status_axis], (tmc2209_regaddr_t)gc_block->values.r);
